...
function SurveyDetails({ URL, token }) {
  ...
  useEffect(() => {
    setComponents(
      data?.surveyQuestions.map((i) => (
        <QuestionComponentSurveyDetails
          key={i._id}
          questionNumber={data.surveyQuestions.indexOf(i) + 1}
          question={i}
        />
      ))
    );
    ...
    if (loader) {
      ...
      return (
        <div className="text-center">
          <Image
            fluid={true}
            src="/assets/img/survey with no answers/Vaya, parece que no has creado una encuesta todavía.png"
          />
        </div>
      );
    } else {
        ...
        <div className="glass-bg p-5">
          <div className="d-md-flex w-100 border-bottom mt-3">
            ...
            <p className="text-light">
              <i className="mx-1 bi bi-clock"></i> Fecha de subida{" "}
  ...
  return (
    <>
      ...
      ;
      <Container
        className="survey-details-components"
